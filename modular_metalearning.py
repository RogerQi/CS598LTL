...
class BounceGrad(object):
  ...
  def log_metrics(self):
    ...
    for i in range(min(len(self.T.MTRAIN), 9)):
      ...
      # TODO: uValOutput has different dim than Input in multivariate case
      ax[i//3, i%3].scatter(dataset.UValInput,
          dataset.UValOutput[:,0], c='g', label='val')
      ax[i//3, i%3].scatter(dataset.UTrainInput,
          dataset.UTrainOutput[:,0], c='r', label='train',
          s=18)
      if self.MAML:
        ax[i//3, i%3].scatter(
          self.T.MTRAIN[self.perm_sample_fns[i]].UValInput,
          self.T.denormalize_output(self.MTrainAnswers[self.perm_sample_fns[i]][1])[:,0]
        )
      else:
      ...
      ax[i//3, i%3].set_ylim([
        np.floor(np.min(self.T.MTRAIN[self.perm_sample_fns[i]].UValOutput[:,0]))-.5,
        np.ceil( np.max(self.T.MTRAIN[self.perm_sample_fns[i]].UValOutput[:,0]))+.5])
      ax[i//3, i%3].set_xticks(np.array([-1,0,1]))
      ax[i//3, i%3].set_yticks(np.array([-1,0,1]))
