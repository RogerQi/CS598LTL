...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, model_type="rnn", n_layers=1,bidir = False):
        ...
        self.bidir = bidir
        if model_type  == "rnn":         
            self.RNN = nn.RNN(input_size,hidden_size,num_layers = n_layers,bidirectional = bidir)
        elif model_type  == "gru":         
            self.RNN = nn.GRU(input_size,hidden_size,num_layers = n_layers,bidirectional = bidir )
#         elif model_type  == "lstm":
#             self.RNN = nn.LSTM(hidden_size,hidden_size,num_layers = n_layers)
        else:
            print("wrong RNN model")
            exit()
        if self.bidir:
    ...
    def forward(self, input, hidden):
        ...
        output,hidden = self.RNN(input.unsqueeze(0).float(),hidden)
